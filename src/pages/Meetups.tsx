// Meetups — maroon #5C0A1E, antique gold #B8860B, white + cream surfaces
    ...
    <div className="min-h-screen" style={{ backgroundColor: '#FDF8F2' }}>
      ...
      <div className="max-w-6xl mx-auto px-6 pt-10 pb-6">
        <div className="flex items-end justify-between mb-6">
          <div>
            <p className="text-xs font-semibold tracking-widest uppercase mb-2" style={{ color: '#5C0A1E' }}>Upcoming</p>
            <h1 className="text-3xl font-bold" style={{ color: '#1A0208' }}>Meetups & Events</h1>
          </div>
          <div className="flex items-center p-1 rounded-full" style={{ backgroundColor: '#fff', border: '0.5px solid #E8DDD5' }}>
            {(['list', 'map'] as const).map((v) => (
              <button key={v} onClick={() => setView(v)}
                className="text-sm px-4 py-1.5 rounded-full transition-colors font-medium capitalize"
      ...
      {view === 'list' ? (
        <div className="max-w-6xl mx-auto px-6 pb-16">
          <p className="text-sm mb-6" style={{ color: '#aaa' }}>{filtered.length} events found</p>
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-xs" style={{ color: '#7A6060' }}>
                      Hosted by <span className="font-medium" style={{ color: '#1A0208' }}>{event.host.name}</span>
                    </span>
                  ...
                  <div className="flex items-center justify-between pt-3" style={{ borderTop: '0.5px solid #E8DDD5' }}>
                    <div>
                      ...
                      <p className="text-xs mt-1" style={{ color: '#aaa' }}>{event.participants}/{event.maxParticipants} joined</p>
                    </div>
        ...
        <div className="max-w-6xl mx-auto px-6 pb-16">
          <div className="w-full h-[500px] rounded-2xl flex items-center justify-center" style={{ backgroundColor: '#fff', border: '0.5px solid #E8DDD5' }}>
            <div className="text-center">
              ...
              <p className="font-medium" style={{ color: '#1A0208' }}>Map view coming soon</p>
              <p className="text-sm mt-1" style={{ color: '#aaa' }}>Google Maps / Mapbox integration pending</p>
            </div>
          </div>
        </div>
      )}
